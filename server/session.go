...
// MafiaSession TODO: extend interface and work with it instead of a struct
...
// TODO: think about changing logic to storing clients on server and using name-Player map in session
...
func (ms *mafiaSession) deliverDelayedNotifications() {
	...
	// TODO: maybe replace with make for the memory issues
...
func (ms *mafiaSession) shuffleRoles() {
	...
	// TODO: think of a more aesthetic shuffling method...
...
func (ms *mafiaSession) runRound() {
	...
	// making all players active at the beginning of the new round
	for _, player := range ms.players {
	...
	if ms.phase == DAY {
		...
		// wait for a bit to ensure, that players are aware of the phase shift
			...
			go func(player MafiaPlayer, mutex *sync.Mutex, wGroup *sync.WaitGroup) {
				...
				if player.GetRole() == DETECTIVE {
					...
					if err != nil {
						log.Println(err.Error())
						player.Notify(Notification{PLAYER_NOT_FOUND, voteRes})
						// TODO: REPLACE WITH continue if outside of goroutine
						wGroup.Done()
						return
					}
		...
		ms.waitGr.Wait()
		ms.carryOutExecution()
		// one round consists of Day and Night
		ms.roundCnt++
		ms.phase = DAY
	}
}
...
func (ms *mafiaSession) Start() {
	...
	ms.phase = DAY
	ms.shuffleRoles()
	// TODO: maybe include optional log in notifyPlayers function
